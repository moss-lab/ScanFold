...
parser.add_argument('--structure_extract_file', type=str, default = "ExtractedStructures.txt",
    ...
    with open(myfasta, 'r') as forward_fasta:
        ...
        ##### Main routine using defined functions: ##########################################
            ...
            #try:
                ...
                se.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (accession, str(int(es.structure_count)+1), str(int((es.i)+1)), str(int((es.j)+1)), str(es.sequence), str(es.structure), str(MFE_structure), str(MFE), str(zscore), str(ED)))
                motif_num += 1
...
### Pulled out fold algorithm and pasted below (for "fold only" functional)
# elif fold_only == True:
...
#                 ED_list.append(ed)
#             #Define OVERALL values of metrics
#             meanz = float(statistics.mean(z_score_list))
#             sdz = float(statistics.stdev(z_score_list))
#             minz = min(z_score_list)
#             stdz = float(statistics.stdev(z_score_list))
#
#             one_sig_below = float(meanz-stdz)
#             two_sig_below = float( meanz - ( 2 * stdz) )
#
...
#             # print(dbn_file_path)
#             # print(dbn_file_path1)
#             # print(dbn_file_path2)
#             # print(dbn_file_path3)
#             # print(dbn_file_path4)
...
#             #refold from -1 constraints
#             fc = RNA.fold_compound(str(full_fasta_sequence), md)
#             dbn_file_filter1 = open(str(dbn_file_path2), "r")
#             lines = dbn_file_filter1.readlines()
#             filter1constraints = str(lines[2])
#             refolded1filter = fc.hc_add_from_db(filter1constraints)
#             (refolded_filter1_structure, refolded_filter1_MFE) = fc.mfe()
...
#             #refold from -2 constraints
#             fc = RNA.fold_compound(str(full_fasta_sequence), md)
#             dbn_file_filter2 = open(str(dbn_file_path3), "r")
#             lines = dbn_file_filter2.readlines()
#             filter2constraints = str(lines[2])
#             refolded2filter = fc.hc_add_from_db(filter2constraints)
#             (refolded_filter2_structure, refolded_filter2_MFE) = fc.mfe()
...
#             dbn_log_file.write(">"+str(name)+"\Refolded with -2 constraints MFE="+str(refolded_filter2_MFE)+"\n"+str(full_fasta_sequence)+"\n"+str(refolded_filter2_structure)+"\n")
#             dbn_log_file.close()
#             os.system(str("cat "+str(dbn_file_path+".dbn")+" "+str(dbn_file_path1+".dbn")+" "+str(dbn_file_path2+".dbn")+" "+str(dbn_file_path3+".dbn")+" > "+str(dbn_file_path4)))
#             dbn_log_file.close()
#         except:
#             print("Automatic refold for "+cur_record.name+" failed. Run manually")
#             os.chdir(str(original_directory))
...
#         #refold from -2 constraints
#         try:
#             dbn_file_filter2 = open(dbn_file_path2+".dbn", "r")
#             lines = dbn_file_filter2.readlines()
#             filter2constraints = str(lines[2])
#             full_fasta_sequence = str(lines[1])
#         except:
#             print("Automatic refold for FoldOnly job failed. Run manually")
#             os.chdir(str(original_directory))
#             del final_partners
#
#     structure_raw = filter2constraints
...
#     #Iterate through sequence to assign nucleotides to structure type
#     m = 0
#     # print(length, str(m)+"=0???")
#     # print(length-1, str(m)+"=0???")            #
#     nuc_dict_refold = {}
#     while  m < length-1:
#         #print(m)
#         if structure[m] == '(':
...
#             pvalue_total.append(pvalue)
#             accession = str(name)
#             ps_title = f"motif_{i} coordinates {i.i} - {i.j}"
#             RNA.PS_rna_plot_a(frag, MFE_structure, "motif_"+str(i), None, ps_title)
#             se.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (accession, str(int(i.structure_count)+1), str(int((i.i)+1)), str(int((i.j)+1)), str(i.sequence), str(i.structure), str(MFE_structure), str(MFE), str(zscore), str(ED)))
#
...
#     se.close()
#     # except:
#     #     print("Structure Extract failed, must extract manually.")
#     #print("TEST")
#     elapsed_time = round((time.time() - start_time), 2)
#     print("Total runtime: "+str(elapsed_time)+"s")
#     print("ScanFold-Fold analysis complete!")
#
#     os.chdir(str(original_directory))
#     del final_partners
else:
    print("error parsing fold argument")
#!/usr/bin/python3
"""
  __    __     ______     ______     ______     __         ______     ______
 /\ "-./  \   /\  __ \   /\  ___\   /\  ___\   /\ \       /\  __ \   /\  == \

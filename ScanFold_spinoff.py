...
if __name__ == "__main__":
    ...
    try:
        ...
        with open(both_args.downloadall, 'wb') as output_wb:
            ...
            tar.add(fold_args.out1, os.path.basename(fold_args.out1))
            tar.add(fold_args.out2, os.path.basename(fold_args.out2))
            tar.add(fold_args.out3, os.path.basename(fold_args.out3))
            tar.add(fold_args.out4, os.path.basename(fold_args.out4))
            tar.add(fold_args.out5, os.path.basename(fold_args.out5))
            tar.add(fold_args.out6, os.path.basename(fold_args.out6))
            tar.add(fold_args.dbn_file_path4, os.path.basename(fold_args.dbn_file_path4))
            tar.add(fold_args.structure_extract_file, os.path.basename(fold_args.structure_extract_file))
            tar.add(fold_args.final_partners_wig, os.path.basename(fold_args.final_partners_wig))
            tar.close()
